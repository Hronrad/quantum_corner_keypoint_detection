...
def extract_extended_structure_tensor_features(patches: np.ndarray, harris_k: float = 0.04) -> np.ndarray:
    """Extract the 8-D QNN improvement feature vector.

    Feature order:
    ``[Ix, Iy, Ix2, Iy2, IxIy, lambda1, lambda2, R]``.
    """
    patches = np.asarray(patches, dtype=np.float32)
    if patches.ndim != 3:
        raise ValueError("patches must have shape (N, H, W).")

    features = np.zeros((patches.shape[0], len(EXTENDED_STRUCTURE_TENSOR_FEATURE_NAMES)), dtype=np.float32)
    for index, patch in enumerate(patches):
        iy, ix = np.gradient(patch)
        center_y = patch.shape[0] // 2
        center_x = patch.shape[1] // 2
